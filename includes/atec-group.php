...
<div class="atec-page">';
	...
	<div class="atec-main">';
		...
		if ($license=='true')
		{
			atec_little_block(__('atec-Plugins „PRO“ package','atec-database'));
			echo '
			<div class="atec-g atec-border atec-center" style="padding: 20px 10px;">
				<h3 class="atec-mt-0">
					...
					esc_attr__('atec-Plugins „PRO“ package','atec-database'), 
				'</h3>
				<div id="atec_group_pro_package">
					<div id="pro_package_welcome" class="atec-fit" style="margin: 0 auto;">
						<div class="atec-mt-5 atec-border-white atec-bg-w atec-fit" style="font-size: 16px !important; padding: 10px;">
							<ul class="atec-m-0">
							<li>⭐ ', esc_attr__('21 valuable plugins','atec-database'), '.</li>					
							<li>⭐ ', esc_attr__('Access to all the „PRO“ features','atec-database'), '.</li>
							<li>⭐ ', esc_attr__('„Lifetime-site-License“ for your site (domain)','atec-database'), '.</li>
							</ul>
						</div>
					</div>
				<br>';
			...
			<button class="button button-primary">', esc_attr__('GET YOUR „PRO“ PACKAGE NOW','atec-database'), '</button></a>
		...
		<div class="atec-g">
			...
			<thead>
				...
				<th></th>
				<th>Name (Link)</th>
				<th>', esc_attr__('Size','atec-database'), '</th>
				<th>Status</th>
				<th>', esc_attr__('Preview','atec-database'), '</th>
				<th>', esc_attr__('Installed','atec-database'), '</th>
				<th>', esc_attr__('Description','atec-database'), '</th>
				</tr>
			</thead>
			<tbody>';
		...
		$atec_review			= ['webp'];
		$atec_desc_arr		= [
							__('All-in-one Backup and restore solution – fast & reliable','atec-database'),
							__('APCu object and page cache','atec-database'),
							__('atec Cache Info & Statistics (OPcache, WP-object-cache, JIT, APCu, Memcached, Redis, SQLite-object-cache)','atec-database'),
							__('Custom code snippets for WP','atec-database'),	
							__('Optimize WP database tables','atec-database'),

							__('Show debug log in admin bar','atec-database'),																	
							__('Install and auto update `atec´ plugins','atec-database'),
							__('Dir Scan & Statistics (Number of files and size per directory)','atec-database'),
							__('Add custom meta tags to the head section','atec-database'),
							__('Lightweight performance tuning plugin','atec-database'),

							__('Measure the PageScore and SpeedIndex of your WordPress site','atec-database'),
							__('Custom translation strings for polylang plugin','atec-database'),
							__('Measure plugins & theme plus pages execution time','atec-database'),	
							__('Add custom SMTP mail settings to WP_Mail','atec-database'),	
							__('Lightweight and GDPR compliant WP statistics','atec-database'),				

							__('System Information (OS, server, memory, PHP and database details, php.ini, wp-config, .htaccess and PHP extensions)','atec-database'),
							__('Ads SVG support for media uploads.','atec-database'),
							__('Manage WordPress User Roles and Capabilities','atec-database'),
							__('Web map, conform with privacy regulations','atec-database'),
							__('Auto convert all images to WebP format','atec-database')
						];
		...
		foreach ($atec_group_arr as $a)
		{
			...
			echo '<tr>
				...
				if ($isWP) echo '
					<td><span title="', esc_attr__('Published','atec-database'), '" class="',esc_attr(atec_dash_class('wordpress')), '"></span></td>
		...
		<center>
			<p class="atec-fs-10" style="max-width:80%;">',
				esc_attr__('All our plugins are optimized for speed, size and CPU footprint (frontend & backend)','atec-database'), '.<br>',
				esc_attr__('Also, they share the same `atec-WP-plugin´ framework – so that shared code will only load once, even with multiple plugins enabled','atec-database'), '.	<br>',
				esc_attr__('With an average of 1 ms CPU time, we consider our plugins to be super fast – other plugins typically require much more time','atec-database'), '.<br>',
				esc_attr__('OSs supported: Linux (CloudLinux, Debian, Ubuntu), Windows & Mac-OS','atec-database'), '.<br>',
				esc_attr__('Webservers tested: Apache, NGINX & LiteSpeed','atec-database'), '.
			</p>
			<a class="atec-nodeco" class="button atec-center" href="https://de.wordpress.org/plugins/search/atec/" target="_blank">', esc_attr__('All atec-plugins in the WordPress directory','atec-database'), '.</a>
		</center>';
	}
